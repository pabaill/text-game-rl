"""
Offline training script for SAC actor model.
Contains logic for:
...
def test_action_generation(actor, llama, test_data, batch_size=32, output_file="test_results.csv"):
    ...
    for index, row in batch_data.iterrows():
        # grab expected state and action from test data
        ...
        # NOTE: deprecated! llama.decode_action() is proper decoding method, used in online training/eval
...
def train(csv_path, 
          ...
          wandb_entity=None):
    """
    Online training for the SAC actor model.
    """
    ...
    # initialize llama, actor, critic, reward shaper, target critic
    llama = LLaMAWrapper()
    ...
    for episode in tqdm(range(episodes), unit="episode"):
        ...
        # sample a batch of data from the csv file for training
        batch_data = data.sample(batch_size)
